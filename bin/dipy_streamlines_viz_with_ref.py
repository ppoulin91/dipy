...
def build_args_parser():
    ...
    p.add_argument("--ref", metavar="tractogram",
                   help="This bundle will be use as model for recobundle. (.trk|.tck).")
...
class Bundle(object):
    ...
    def _cluster(self, threshold):
        ...
        self.clusters_colors = [color for c, color in zip(self.clusters, distinguishable_colormap(bg=(0, 0, 0), exclude=[(1, 1, 1)]))]
    ...
    def keep_close_to(self, ref, threshold):
        if self.actor is not None:
            self.ren.rm(self.actor)

        if self.actor_unassigned is not None:
            self.ren.rm(self.actor_unassigned)

        # Recobundle like
        qb = QuickBundles(metric=metric, threshold=threshold)
        clusters, unassigned = qb.assign(ref.clusters, self.streamlines)

        streamlines = nib.streamlines.ArraySequence()
        for c in clusters:
            streamlines.extend(self.streamlines[c.indices])

        unassigned_streamlines = self.streamlines[unassigned.indices]

        # Create 3D actor to display this bundle's streamlines.
        self.actor = None
        if len(streamlines) > 0:
            self.actor = actor.line(streamlines, colors=(1, 0.5, 0))
            self.ren.add(self.actor)

        self.actor_unacd ssigned = None
        # if len(unassigned_streamlines) > 0:
        #     self.actor_unassigned = actor.line(unassigned_streamlines, colors=None)
        #     self.ren.add(self.actor_unassigned)

class StreamlinesVizu(object):
    # def __init__(self, tractogram_filename, savedir="./", screen_size=(1024, 768)):
    def __init__(self, tractogram, ref, prefix="", screen_size=(1360, 768)):
        self.prefix = prefix
        ...
        self.ref = Bundle(ref.streamlines, color=(1, 1, 1))
        self.tractogram = Bundle(tractogram.streamlines)

        self.bundles = {}
        self.root_bundle = "/"
        self.bundles[self.root_bundle] = self.tractogram
    ...
    def select(self, bundle_name=None):
        ...
        self.clustering_panel.slider.max_value = 0
        ...
        bundle.keep_close_to(self.ref, threshold=self.clustering_panel.slider.value)
    ...
    def _make_clustering_panel(self):
        ...
        def apply_button_callback(iren, obj, button):
            ...
            # obj: vtkActor picked
            # button: Button2D
            bundles = self.bundles[self.selected_bundle].get_filter_as_bundles()
        ...
        def disk_move_callback(iren, obj, slider):
            ...
            if self.last_threshold != threshold:
                self.bundles[self.selected_bundle].keep_close_to(self.ref, threshold)
                self.last_threshold = threshold
...
def main():
    ...
    tractogram_ref = nib.streamlines.load(args.ref)

    vizu = StreamlinesVizu(tractogram, ref=tractogram_ref, prefix=prefix)
